        ...
        Kviff.Data.Screening_ b ->
            column [ spacing 4, Kviff.ElementId.toId (Kviff.ElementId.Event id) ]
                [ column [ spacing 2 ]
                    [ heading2 theme
                        [ fontSemiBold ]
                        [ text (Kviff.Locale.localize model.locale b.name)
                        ]
                    , paragraph theme
                        [ fontSize 14, fontColor style.fore70 ]
                        ([ case b.type_ of
                            ...
                            _ ->
                                Just (text (Translation.eventType b.type_))
                         , b.startTime
                         ...
                         , Just
                            (case b.place.gps of
                                Just b ->
                                    newTabLink []
                                        { label = text (Api.localize model.locale b.place.name)
                                        , url =
                                ...
                                Nothing ->
                                    text (Api.localize model.locale b.place.name)
                            )
                         , case b.filmId of
                            Just b ->
                                Just
                         ...
                         , case b.filmId of
                            Just _ ->
                                Just
                                    (newTabLink []
                                        { label = text "CSFD"
                                        , url = "https://www.csfd.cz/hledat/?q=" ++ Url.percentEncode b.name.cz
                                        }
                                    )
                ...
                , p [ fontSize 14, fontColor grey3 ]
                    [ text
                        (Utils.Html.stripTags (Api.localize model.locale b.description)
                            |> Result.withDefault (Api.localize model.locale b.description)
                        )
                    ]
                , p [ fontSize 10, fontColor grey4 ]
                    [ text (Api.localize model.locale b.note)
                    ]
                ]
